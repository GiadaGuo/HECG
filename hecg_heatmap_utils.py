...
#将输入的注意力分数（attns）转换为一个标准化的分数图（color_block），并将其大小调整为目标大小（默认为 256x256）
def get_scores256(attns, size=(256,256)):
    r"""
    """

    #匿名函数rank：将每个值转换为其在数组中的相对排名，并归一化到 [0, 100] 的范围内
    rank = lambda v: rankdata(v)*100/len(v)
...
def create_patch_heatmaps_concat(patch, model256, output_dir, fname, threshold=0.5,
    ...
    """
    patch1 = patch.copy()
    #patch2: 是裁剪后的图像，并在其周围添加了边距，作用暂时不明
    patch2 = add_margin(patch.crop((16,16,256,256)), top=0, left=0, bottom=16, right=16, color=(255,255,255))

    #b256：返回numpy image array格式的图像；
    #a256：返回CLS token 对所有 patch 的注意力分数
    b256_1, a256_1 = get_patch_attention_scores(patch1, model256, device256=device256)
    b256_2, a256_2 = get_patch_attention_scores(patch2, model256, device256=device256)

    save_region = np.array(patch.copy()) #将输入图像 patch 转换为 NumPy 数组，存储在 save_region 中
    s = 256
    offset_2 = offset
    ...
    if threshold != None:
        ...
        for i in range(6):
            ...
            #调用 get_scores256 函数
            #将注意力分数 a256_1 和 a256_2 转换为大小为 (256, 256) 的分数图
            score256_1 = get_scores256(a256_1[:,i,:,:], size=(s,)*2)
            score256_2 = get_scores256(a256_2[:,i,:,:], size=(s,)*2)
            ...
            #建一个与 score256 大小相同的掩码 mask256，并将其值初始化为 score256 的值。
            ##将 mask256 中小于阈值 threshold 的值设为 0。
            ##将 mask256 中大于阈值 threshold 的值设为 0.95
            mask256 = score256.copy()
            mask256[mask256 < threshold] = 0
            mask256[mask256 > threshold] = 0.95
