...
#通过一个 ViT-256 模型（model256）对输入的图像块（patch）进行前向传播，并返回注意力分数和处理后的图像块
def get_patch_attention_scores(patch, model256, scale=1, device256=torch.device('cpu')):
    ...
    with torch.no_grad():   
        batch_256 = t(patch).unsqueeze(0) #unsqueeze(0): 在第 0 维度添加一个额外的维度，将单个图像转换为一个批量（batch）
        batch_256 = batch_256.to(device256, non_blocking=True)
        features_256 = model256(batch_256) #为什么没有用？

        #获取模型的最后一个自注意力层的注意力分数
        #attention_256含义：ViT 认为哪些 patch 对整张图片最重要
        attention_256 = model256.get_last_selfattention(batch_256) #可能输出结构：(batch_size, num_heads, seq_length, seq_length)
        nh = attention_256.shape[1] # number of head：获取注意力分数的维度，表示注意力头的数量。
        attention_256 = attention_256[:, :, 0, 1:].reshape(256, nh, -1) #取 CLS token 对所有 patch 的注意力
        attention_256 = attention_256.reshape(1, nh, 16, 16) #每个注意力头的注意力分数是一个 16x16 的矩阵
        #nn.functional.interpolate：对注意力分数进行上采样或下采样，使其分辨率与目标分辨率一致
        attention_256 = nn.functional.interpolate(attention_256, scale_factor=int(16/scale), mode="nearest").cpu().numpy()

        #将输入图像的分辨率调整为与注意力分数的分辨率一致
        if scale != 1:
            batch_256 = nn.functional.interpolate(batch_256, scale_factor=(1/scale), mode="nearest")

    #tensorbatch2im(batch_256)：Converts a Tensor array into a numpy image array
...
def get_region_attention_scores(region, model256, model4k, scale=1,
    ...
    with torch.no_grad():
        #(3,4096,4096)->(1,3,4096,4096)
        #把 4096×4096 图片切成 256×256 的小块：(1,3,4096,4096)->(1, 3, 16, 16, 256, 256)——2个unfold
        batch_256 = t(region).unsqueeze(0).unfold(2, 256, 256).unfold(3, 256, 256)
        #(1, 3, 16, 16, 256, 256) -> (256, 3, 256, 256)——每个 256*256 patch 独立送入模型
        batch_256 = rearrange(batch_256, 'b c p1 p2 w h -> (b p1 p2) c w h')
        batch_256 = batch_256.to(device256, non_blocking=True)
        features_256 = model256(batch_256) #(256×384)

        #获取 ViT-256 最后一层的注意力分数
        attention_256 = model256.get_last_selfattention(batch_256)
        nh = attention_256.shape[1] # number of head
        attention_256 = attention_256[:, :, 0, 1:].reshape(256, nh, -1) #取 CLS Token 对所有 patch 的注意力（CLS Token 代表全局信息）
        attention_256 = attention_256.reshape(256, nh, 16, 16) #变成 每个 patch 一个 16×16 的注意力矩阵。
        attention_256 = nn.functional.interpolate(attention_256, scale_factor=int(16/scale), mode="nearest").cpu().numpy() #调整注意力图大小，适配 scale

        #组合 256×256 patch 特征，输入 ViT-4096

        #features维度变换：(256×384)->(16×16×384)->(1×16×16×384)
        ...
        attention_4096 = attention_4096[0, :, 0, 1:].reshape(nh, -1)   #取出 CLS Token 的注意力
        attention_4096 = attention_4096.reshape(nh, 16, 16)   #转换成 16×16 矩阵
        attention_4096 = nn.functional.interpolate(attention_4096.unsqueeze(0), scale_factor=int(256/scale), mode="nearest")[0].cpu().numpy() #处理注意力分辨率
